...
def wallet_modal():
    ...
    name_ent = tk.Entry(w, width=35); name_ent.insert(0, "new_wallet"); name_ent.pack(pady=5)
    add_right_click(name_ent) # Pridedam meniu

    tk.Label(w, text="Password:").pack(); 
    pw = tk.Entry(w, show="*", width=35); pw.pack(pady=5)
    add_right_click(pw) # Pridedam meniu

    def go(event=None): # event=None reikalingas dėl Enter paspaudimo
        if not pw.get() or not name_ent.get():
            messagebox.showwarning("Error", "Enter filename and password!"); return

        base_dir = os.path.join(os.getcwd(), "data", "sn01")
        os.makedirs(base_dir, exist_ok=True)
        pwd_file = os.path.join(base_dir, ".tmp_pwd")
        with open(pwd_file, "w") as f: f.write(pw.get())
    ...
    # Pririšame Enter klavišą prie abiejų laukelių
