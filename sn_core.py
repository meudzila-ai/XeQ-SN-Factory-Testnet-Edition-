...
def create_compose(base, count, ip):
    ...
    # Removed 'version' to avoid Docker warnings
...
def get_registration(selected_indices, amount, wallet):
    ...
    for i in selected_indices:
        ...
        try:
            # Added a short retry or check for the node response
            ...
            if "result" in r:
                ...
                err = r.get("error", {}).get("message", "Node is starting up or not synced.")
                results.append(f"SN{i+1:02d} Error: {err}")
        except Exception as e:
            results.append(f"SN{i+1:02d} Error: Node unreachable on port {rpc}. Wait 30s and try again.")
    return results